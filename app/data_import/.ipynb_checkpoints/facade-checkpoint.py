...
class TickerQuote(NamedTuple):
    ...
    @classmethod
    def from_yahoo_quote(cls, record:dict) -> 'TickerQuote':
        return TickerQuote(
...
class DataFacade:
    ...
    def get_weekly_price(
        ...
        if provider == 'yahoo':
            ...
            for chunk in more_itertools.chunked(adjusted_history, chunk_size):
                yield chunk
        else:
            raise Exception(f'provider not implemented: {provider}.')

        
    def get_intra_day_price(self, symbol:str, interval='5m', start_date: Optional[date] = None, end_date: Optional[date] = None, provider='yahoo', chunk_size=5000) -> Iterable[List[TickerQuote]]:
        start_date = start_date if start_date else datetime.date.today() - datetime.timedelta(days=7)
        end_date = end_date if end_date else datetime.date.today()
        ...
        if provider == 'yahoo':
            ...
            for i in range(batches):
                ...
                for chunk in more_itertools.chunked(map(lambda x: TickerQuote.from_yahoo_quote(x), adjusted_history), 5000):
                    yield chunk
        else:
            raise Exception(f'provider not implemented: {provider}.')
